...
func (pmq *PaymentMethodQuery) QueryTransactions() *TransactionQuery {
	query := (&TransactionClient{config: pmq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pmq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pmq *PaymentMethodQuery) Clone() *PaymentMethodQuery {
	...
	return &PaymentMethodQuery{
		...
		withTransactions: pmq.withTransactions.Clone(),
...
func (pmq *PaymentMethodQuery) WithTransactions(opts ...func(*TransactionQuery)) *PaymentMethodQuery {
	query := (&TransactionClient{config: pmq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pmq.withTransactions = query
	return pmq
}
...
//		GroupBy(paymentmethod.FieldUserID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.PaymentMethod.Query().
//		Select(paymentmethod.FieldUserID).
//		Scan(ctx, &v)
...
func (pmq *PaymentMethodQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PaymentMethod, error) {
	...
	if query := pmq.withTransactions; query != nil {
		if err := pmq.loadTransactions(ctx, query, nodes,
			func(n *PaymentMethod) { n.Edges.Transactions = []*Transaction{} },
			func(n *PaymentMethod, e *Transaction) { n.Edges.Transactions = append(n.Edges.Transactions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pmq *PaymentMethodQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*PaymentMethod, init func(*PaymentMethod), assign func(*PaymentMethod, *User)) error {
	...
	nodeids := make(map[string][]*PaymentMethod)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (pmq *PaymentMethodQuery) loadTransactions(ctx context.Context, query *TransactionQuery, nodes []*PaymentMethod, init func(*PaymentMethod), assign func(*PaymentMethod, *Transaction)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(transaction.FieldPaymentMethodID)
	}
	query.Where(predicate.Transaction(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(paymentmethod.TransactionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err

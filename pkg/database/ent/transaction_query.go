...
func (tq *TransactionQuery) QueryPaymentMethod() *PaymentMethodQuery {
	query := (&PaymentMethodClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TransactionQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (tq *TransactionQuery) Clone() *TransactionQuery {
	...
	return &TransactionQuery{
		...
		withPaymentMethod: tq.withPaymentMethod.Clone(),
...
func (tq *TransactionQuery) WithPaymentMethod(opts ...func(*PaymentMethodQuery)) *TransactionQuery {
	query := (&PaymentMethodClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withPaymentMethod = query
	return tq
}
...
//		GroupBy(transaction.FieldUserID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Transaction.Query().
//		Select(transaction.FieldUserID).
//		Scan(ctx, &v)
...
func (tq *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if query := tq.withPaymentMethod; query != nil {
		if err := tq.loadPaymentMethod(ctx, query, nodes, nil,
			func(n *Transaction, e *PaymentMethod) { n.Edges.PaymentMethod = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TransactionQuery) loadPaymentMethod(ctx context.Context, query *PaymentMethodQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *PaymentMethod)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Transaction)
	for i := range nodes {
...
func (tq *TransactionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withPaymentMethod != nil {
			_spec.Node.AddColumnOnce(transaction.FieldPaymentMethodID)
		}
	}
